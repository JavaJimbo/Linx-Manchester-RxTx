...
unsigned short TotalBytesReceived = 0;
...
int main(void) {
    ...
    while (1) {
        if (previousExpected != numExpectedBytes){
            previousExpected = numExpectedBytes;
            printf("\rExpected: %d", numExpectedBytes);
        }
        if (numDataBytes) {
            if (numDataBytes < 8)
                ...
                printf("\r\rTotal Bytes: %d, ", (int) TotalBytesReceived);
                TotalBytesReceived = 0;
                printf("#%d: BYTES: %d, DATA: ", ++trialCounter, numDataBytes);
                for (i = 0; i < numDataBytes; i++) {
                    if ((i % 8) == 0)printf("\r%X, ", arrData[i]);
                    else printf("%X, ", arrData[i]);
                }
                if (convert.CRCinteger != CRCcheck) printf("\r\rCRC ERROR");
            }
            if (timeoutFlag) {
                printf("\rTIMEOUT");
                timeoutFlag = FALSE;
